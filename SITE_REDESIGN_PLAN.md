...
**Deploy break caused by this work, now fixed**: `netlify-cli` was added as an `angular/package.json` **devDependency** so `netlify dev` would be available locally — but Netlify's own build installs *all* dependencies (dev included), and `netlify-cli@27.1.2` requires Node ≥22.13 while the site's build image runs Node 18.20.8. First deploy after this work failed at `Install dependencies` with `error netlify-cli@27.1.2: The engine "node" is incompatible with this module`. **Fixed** by removing it from `package.json`/`yarn.lock` entirely (`yarn remove netlify-cli`) — it should never have been a committed project dependency. For local use going forward, run it ad-hoc instead: `npx netlify-cli dev` (downloads/caches on first use, doesn't touch the build). Lesson: any future local-only tooling belongs outside the deployed dependency tree, not in `dependencies`/`devDependencies`.
